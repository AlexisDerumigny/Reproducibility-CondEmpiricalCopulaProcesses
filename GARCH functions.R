...
#
# https://doi.org/10.1590/1982-7849rac2021200088
#
...
do_sim <- function(n_sim = 1000, n_t = 1000, my_garch, df_prices) {
  ...
  df_out <- bind_rows(map(.x = 1:n_sim, .f = do_single_sim,
                          my_garch = my_garch, n_t = n_t,
                          df_prices=df_prices))
...
find_best_arch_model <- function(x, max_global_lag = 5) {
  ...
  df_grid <- expand_grid(arma_lag = 0:max_global_lag,
                         ma_lag = 0:max_global_lag,
                         arch_lag = 0:max_global_lag,
...
do_single_garch <- function(x, lag_ar, lag_ma, lag_arch, lag_garch) {
  ...
  my_formula <- formula(paste('~arma(', lag_ar, ',',lag_ma,')', ' + ',
  ...
  try({
    my_garch <- garchFit(formula = my_formula,
                         data = x, trace = FALSE)
  }, silent = TRUE)
